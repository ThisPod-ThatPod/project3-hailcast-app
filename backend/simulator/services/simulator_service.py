...
class SimulatorService:
    def __init__(self, state: SimulatorState, runner: K6Runner, settings: SimulatorSettings):
        ...
        # httpx timeout=5.0은 connect/read/write/pool 전 구간에 동일하게 적용되는 총 타임아웃이라,
        # 의미가 가장 가까운 aiohttp의 total로 옮긴다(connect/sock_read 등 세부 타임아웃은 안 씀).
        timeout = aiohttp.ClientTimeout(total=settings.relay_timeout_seconds)
    ...
    async def relay_call(self, body: bytes) -> tuple[int, bytes]:
        ...
        try:
            async with self._relay_client.post(
