...
class IntervalScheduler(ABC):
    ...
    def __init__(
        ...
        align_to_seconds: float | None = None,
        align_offset_seconds: float = 0.0,
    ):
        """align_to_seconds를 주면 UTC 자정 기준 그 배수 지점(+align_offset_seconds)에
        정확히 맞춰 첫 실행을 시작한다 (예: interval=4h, align_to=4h, offset=3min →
        00:03/04:03/08:03... UTC). Unix epoch가 UTC 자정이라 별도 기준시각 계산 없이
        `now % align_to_seconds`만으로 정렬된다. 미지정 시 기존처럼 즉시 시작."""
        self._interval = interval_seconds
        self._align_to = align_to_seconds
        self._align_offset = align_offset_seconds
        self._running = False
    ...
    def _seconds_until_next_boundary(self) -> float:
        if self._align_to is None:
            return 0.0
        phase = (time.time() - self._align_offset) % self._align_to
        return (self._align_to - phase) % self._align_to
    ...
    async def run_loop(self) -> None:
        ...
        if delay:
            logger.info(
                f"{self.name} waiting for aligned start ({delay:.0f}s)",
                extra={"event": "scheduler_aligning", "detail": {"scheduler": self.name}},
            )
