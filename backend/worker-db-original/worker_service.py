...
# FileStore 버전(C, 2026-07-13). J3(DB 이번 페이즈에서 뺀다)와 이 다이어그램이 충돌해서
# 둘 다 만들어두기로 함 — RDS가 실제로 준비되면 이 폴더 내용을 원래 자리로 되돌리고
# dependencies.py만 Database 기반으로 바꾸면 된다.
#
# WorkerService — SQS 소비 Business Logic
# 흐름: Polling → Deserialize → Validation → DB Save → ACK(delete)
