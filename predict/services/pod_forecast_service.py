...
#
# 시간 구간별 데이터 출처:
#   - 과거(bucket < 현재 정시)  : PodReplicaHistory 백업 테이블 (BackupScheduler가 매시 정각 스냅샷)
#   - 현재(bucket == 현재 정시) : S3 predictions/latest.json(실시간) + KedaAdapter 실제 파드 수(실시간)
#   - 미래(bucket > 현재 정시)  : S3 predictions/latest.json의 해당 horizon step (predicted만, actual 없음)
...
from repositories.pod_replica_repository import PodReplicaRepository
...
class PodForecastService:
    ...
    def _nearest_total(
        self, totals: dict[datetime, float], bucket: datetime
    ) -> float | None:
        """가장 가까운 target_time을 버킷에 매칭한다.

        target_time은 forecast 실행 시각(now)에 고정돼 정시 버킷과 최대 거의 window 크기만큼
        어긋날 수 있다 (forecast_interval < window인 한, window 전체를 허용오차로 잡으면 항상 매칭된다).
        """
        if not totals:
            return None
    ...
    def snapshot_current_hour(self) -> None:
        ...
        try:
            ...
            logger.warning(
                f"actual replica scrape failed: {exc}",
                extra={"event": "pod_backup_partial"},
            )

        with self._db.session_scope() as session:
            PodReplicaRepository(session).upsert(
                bucket,
        ...
        logger.info(
            f"pod history snapshot saved (bucket={bucket.isoformat()}, "
    ...
    def pod_forecast(self, hours_history: int, hours_forecast: int) -> list[PodForecastPoint]:
        ...
        with self._db.session_scope() as session:
            history_rows = PodReplicaRepository(session).get_range(start, now_bucket)
        history_by_bucket = {
            r.bucket_time: (r.predicted_replicas, r.actual_replicas) for r in history_rows
        }
