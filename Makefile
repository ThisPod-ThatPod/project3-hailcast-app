...
help: ## 명령 목록
	...
	@echo "====================================================="
	@echo ""
	@echo "  ⚠️  실제 EKS 운영 환경에서는 이 레포의 배포·정리가 '공식 경로'가 아닙니다."
	@echo "      - image build/push 공식 경로 : GitHub Actions (.github/workflows/build.yml)"
	@echo "      - EKS 워크로드 정리 공식 경로 : manifests 레포 Makefile (make teardown 등)"
	@echo "      아래 [ 운영 ] 항목은 로컬 수동 실행/디버깅용입니다. 자세한 내용: scripts/README.md"
	@echo ""
	...
	@echo ""
	@echo "  [ 운영 (AWS · 계정 가드 선행) — ⚠️ 공식 경로 아님, 로컬 수동용 ]"
	@echo "  make build-push      docker build → ECR push (TAG=... 지정 가능)"
	@echo "                       ⚠️ 운영 배포는 GitHub Actions 가 자동으로 한다. 이건 로컬 수동/디버깅용."
	@echo "  make deploy-all      실인프라 배포 전체 자동화 (kubeconfig→build-push→"
	@echo "                       AWS 값 조회→k8s apply→기동 대기, 재실행해도 안전)"
	@echo "                       ⚠️ ArgoCD 가 이미 대상 워크로드를 관리 중이면 selfHeal 로 되돌려질 수 있다."
	@echo ""
	@echo "  [ 정리 ]"
	@echo "  make teardown        로컬 도커 이미지·볼륨·캐시 정리 (CONFIRM=yes 시 실제 삭제)"
	@echo "  make teardown-infra  ⚠️ 공식 정리 경로 아님 — EKS 워크로드 정리는 manifests 레포"
	@echo "                       Makefile 이 공식 경로다. 실인프라(EKS) 배포분 전부 삭제 —"
	@echo "                       hailcast 네임스페이스 + 클러스터 RBAC (CONFIRM=yes 시 실제 삭제)"
	@echo ""
...
# ⚠️ 공식 경로 아님 — 운영 배포는 GitHub Actions(.github/workflows/build.yml)가 자동으로 한다.
#    여기는 로컬 수동 실행/디버깅용이다. 자세한 내용: scripts/README.md
# 계정 가드·ECR 로그인·리포지토리 존재 확인은 스크립트 안에서 한다.
build-push: ## docker build → ECR push (⚠️ 공식 경로 아님 — 운영은 GitHub Actions)
	@echo "⚠️  공식 배포 경로 아님 — 운영 이미지 build/push 는 GitHub Actions 가 자동으로 합니다."
	@echo "    로컬 수동/디버깅용으로만 쓰세요. 자세한 내용: scripts/README.md"
	@SERVICES="$(SERVICES)" AWS_REGION="$(REGION)" bash scripts/build_push.sh

# ── 운영 : 실인프라 배포 전체 자동화 (scripts/deploy_infra.sh) ──
# ⚠️ 공식 경로 아님 — ArgoCD 가 이미 대상 워크로드를 관리 중이면 selfHeal 로 되돌려질 수 있다.
# docs/2026-07-20-app-deploy-guide.md 의 0~4단계를 한 번에 실행한다.
deploy-all: ## 실인프라 배포 한 번에 (⚠️ 공식 경로 아님 — ArgoCD 가 selfHeal 로 되돌릴 수 있음)
	@echo "⚠️  공식 배포 경로 아님 — ArgoCD 가 이미 관리 중인 리소스는 selfHeal 로 되돌아갈 수 있습니다."
	@echo "    로컬 수동/디버깅용으로만 쓰세요. 자세한 내용: scripts/README.md"
...
# ⚠️ 공식 정리 경로 아님 — EKS 워크로드 정리는 manifests 레포 Makefile 이 공식 경로다.
# 기본은 미리보기만. 실제 삭제는 CONFIRM=yes.
teardown-infra: ## 실인프라 배포분 삭제 (⚠️ 공식 경로 아님 — manifests 레포 Makefile 이 공식)
	@echo "⚠️  공식 정리 경로 아님 — EKS 워크로드 정리는 manifests 레포 Makefile 이 공식 경로입니다."
	@echo "    자세한 내용: scripts/README.md"
